...
public class Start extends Application {
	...
	private List<Order> readOrdersFromSource() {
		if(USE_DEFAULT_DATA) {
    		...
    		try {
    			...
    			stmt = con.createStatement();
    			ResultSet rs = stmt.executeQuery("SELECT * FROM Ord where custid=1");

    			while(rs.next()){
    				List<OrderItem> listOI =readfromOrderItem(1);
	...
	private List<OrderItem> readfromOrderItem(int orderid) {
		...
		try {
			con = ConnectManager.getConnection(ConnectManager.DB.ACCT);
			stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT * FROM orderitem where orderid=1");
			...
			while(rs.next()){
                ...
                List<String> temp=readDataFromProduct(Integer.parseInt(productid2));
                String productname=temp.get(0);
                String unitprice=temp.get(1);

                	list.add(new OrderItem(productname,quantity,unitprice));
     ...
     System.out.println("OrderItem working");
	return list;
}



	private List<String> readDataFromProduct(int productid2) {
    		...
    		try {
    			con = ConnectManager.getConnection(ConnectManager.DB.PROD);
    			stmt = con.createStatement();
    			ResultSet rs = stmt.executeQuery("SELECT * FROM product where productid=1");

    			while(rs.next()){


                    	String productname=rs.getString("productname");
                    	String unitprice=rs.getString("priceperunit");
         ...
         System.out.println("product workinng");
		return list;
	}
